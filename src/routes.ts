...
routes.get(path, (req, res, next) => {
    authController.authMiddleware(req, res, next); // Call authMiddleware first
}, (req, res, next) => {
    checkingAccountController.getAll(req, res).catch(next); // Handle errors with next()
});
...
routes.put(`${path}/:id`, (req, res, next) => { authController.authMiddleware }, (req, res, next) => {
    checkingAccountController.verifyIfExists(req, res, next); // Assuming this is middleware too
}, (req, res, next) => {
